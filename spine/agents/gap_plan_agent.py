...
def build_gap_plan_agent(
    ...
    system_prompt = (
        ...
        "3. Diagnose the ROOT CAUSE by inspecting ``target_file_sources`` — the "
        "CURRENT content of every feature file. The failure often surfaces in one "
        "slice but is caused by a DIFFERENT slice's file (e.g. a test crashes on a "
        "missing table because the MODEL file lacks the $table the migration "
        "created). Trace the failing evidence back to the file that must change; "
        "do not stop at the file where the error appeared.\n"
        "4. Produce a structured gap_plan.md that tells the implement phase "
        "EXACTLY what to fix. For each issue, specify:\n"
        "   - The slice_id that OWNS the file needing the change — attribute the "
        "fix to the producing slice even if that slice already passed "
        "verification (passed slices are reopened when a fix targets them)\n"
