...
def _build_specify_prompt() -> str:
    return (
        ...
        "Your tools are restricted to `read_work_context`, `write_specification`, "
        "`task`, and `eval`. This is enforced by tool design — you cannot call "
        "`ls`, `read_file`, `glob`, `grep`, `write_file`, `edit_file`, or `execute`. "
        "Use `researcher` subagents for codebase exploration.\\n\\n"
        ...
        "Avoid thin dispatches like 'Research X' — these produce empty results "
        "and waste a subagent turn (costing 3K tokens for nothing).\\n\\n"
        "Dispatch pattern:\\n"
        "```js\\n"
        ...
        "## Strict rules\\n"
        "- Call `read_work_context` first. It provides the work description and context.\\n"
        "- Dispatch researchers before writing. Synthesize findings into a specification.\\n"
        "- Call `write_specification` once, with all required fields.\\n"
        "- Target ~3 turns. If you reach 5+ turns without calling "
        "`write_specification`, something has gone wrong.\\n\\n"
        "## Eval context seed\\n"
