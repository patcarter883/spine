...
# Shared structured-plan field contract. Both the orchestrator and the
# synthesizer emit the same plan schema, so the field lists live in one place.
_PLAN_TOP_FIELDS = (
    "Top-level fields:\n"
    "- architecture_overview: prose paragraph (string) on how the components "
    "fit together\n"
    "- technology_choices: list of short strings, one item per choice "
    "(rationale inline)\n"
    "- feature_slices: list of structured slice objects (see below). REQUIRED "
    "— at least one.\n"
...
_PLAN_SLICE_FIELDS = (
    ...
    "- reference_symbols: qualified names of EXISTING symbols the slice's code "
    "will call/extend/mimic (e.g. 'UIApi.update_mcp_server', 'SpineConfig') — "
    "take these from your codebase research so the implementer reads exactly "
    "those instead of surveying files\n"
    "- dependencies: list of slice IDs that must complete first. Every ID here "
    "MUST be the id of another slice in THIS plan — never reference a slice you "
    "did not define.\n"
...
def _build_plan_prompt() -> str:
    return (
        ...
        "## Tools\n"
        "- `read_prior_artifacts` — loads spec and all prior artifacts. Call "
        "FIRST.\n"
        "- `search_codebase` — targeted file search for narrow lookups not in "
        "exploration.\n"
        "- `write_structured_plan` — emits feature_slices. Call LAST.\n\n"
        "## Workflow (~3 turns)\n"
        "1. Call `read_prior_artifacts` (no arguments) to load the "
        "specification and prior exploration results.\n"
        "2. Codebase research already ran via the exploration subgraph; its "
        "findings are in your context. Synthesise them — do NOT dispatch "
        "researchers. Use `search_codebase` only for narrow lookups (specific "
        "paths/symbols) not already covered.\n"
        "3. Synthesize spec + exploration into the structured fields below and "
        "call `write_structured_plan` ONCE. " + NO_MARKDOWN_WRITE_NOTE + "\n\n"
        + _PLAN_TOP_FIELDS + "\n\n"
        + _PLAN_SLICE_FIELDS + "\n\n"
        "## Rework handling\n"
        "If feedback exists in prior artifacts, address every item before "
        "calling `write_structured_plan`.\n\n"
        "## Rules\n"
        "- Call `read_prior_artifacts` first.\n"
        "- Every target_file path must come from `search_codebase`, the "
        "exploration findings in your context, or a confirmed existing "
        "directory.\n"
        "- Call `write_structured_plan` exactly once with all required fields.\n"
        "- Do not stall: by turn 4, call `write_structured_plan` covering every "
        "spec requirement with the information you have — note residual "
        "uncertainty in `risks` rather than dropping slices.\n"
    )
...
def _build_plan_synthesizer_prompt() -> str:
    return (
        ...
        "Your job is to synthesize spec + findings into a structured plan and "
        "call `write_structured_plan` ONCE. Do NOT re-explore the codebase.\n\n"
        "## Proportionality\n"
        "Use the FEWEST slices that still cover EVERY spec requirement. Extend "
        "existing patterns and fold related work into one slice rather than "
        "adding modules or slices the spec does not require — but never drop a "
        "requirement: if the spec asks for tests or acceptance criteria, the "
        "slice MUST list the test file(s) in `target_files` and encode them in "
        "`acceptance_criteria`. Every slice traces to a requirement, and every "
        "requirement is covered by a slice.\n\n"
        "## Tools\n"
        "- `write_structured_plan` — writes plan.md + plan.json. It is the ONLY "
        "tool you have.\n\n"
        "## Workflow (exactly 1 call)\n"
        "Everything you need is ALREADY in your prompt: the specification "
        "(<specification> block), the research findings (<findings> block), and "
        "the objective. There is nothing to read or load first. Synthesize spec "
        "+ findings into structured fields and call `write_structured_plan` "
        "ONCE. " + NO_MARKDOWN_WRITE_NOTE + "\n\n"
        + _PLAN_TOP_FIELDS + "\n\n"
        + _PLAN_SLICE_FIELDS + "\n"
    )
