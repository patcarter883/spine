    ...
    *,
    ...
    if spec_payload:
        spec_contradiction_note = (
            ...
            "(a named method, property, or idiom) where the target "
            "framework's standard convention delivers the same behavior is "
            "a BLOCKING plan defect, fixed by restating the criterion as "
            "the observable OUTCOME (run a56e89a6: 'bind the model via a "
            "model() method' parked a run whose editor correctly used "
            "Laravel's protected $model convention). VERIFY will "
            "enforce every criterion literally, so an ungrounded, "
            "contradictory, or prescriptive criterion strands the run."
