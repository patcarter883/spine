...
For each node_output:
    - Extract artifacts dict (node_output.get("artifacts"))
    - Deep-merge into local result dict
    - ✅ NEW: Save artifacts immediately via artifacts.save_artifact()
...
async for chunk in graph.astream(initial_state, thread_config):
    for node_name, node_output in chunk.items():
        ...
        # NEW: Save artifacts immediately after merge
        _save_node_artifacts(work_id, node_artifacts, artifacts)

# POST-LOOP: Fallback save (kept for safety)
for phase, phase_artifacts in result_artifacts.items():
    ...
```
...
├── NEW Line 211: _save_node_artifacts() call (after result.update)
├── Lines 229-233: Post-loop fallback (retain for safety)
└── NEW Lines 89-90: _save_node_artifacts() helper function (module-level)
...
# NEW LINE 211-212: Save artifacts immediately after merge
_save_node_artifacts(work_id, node_artifacts, artifacts)

# Lines 211-222: Progress update (existing, keep as-is)
...
| 2 | Insert call in astream loop | dispatcher.py | 211 | Step 1 |
...
1. **Line 210** is the insertion point (after `result.update(node_output)`)
   ...
   - The `node_artifacts` variable from line 194 is available for use
